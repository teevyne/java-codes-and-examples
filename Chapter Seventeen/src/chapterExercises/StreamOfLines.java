...
public class StreamOfLines
{
	public static void main(String[] args) throws IOException
	{
		...
		Map<String, Long> wordCounts = Files.lines(Paths.get("C:\\Users\\user\\eclipse-workspace\\"
				+ "Chapter Seventeen\\src\\testfile.txt")).map(line -> line.replaceAll("(?!')\\p{P}", ""))
			.flatMap(line -> pattern.splitAsStream(line))
			.collect(Collectors
			.groupingBy(String::toLowerCase, TreeMap::new, Collectors.counting()));
 
		// display the words grouped by starting letter
		wordCounts.entrySet().stream().collect(Collectors.groupingBy(entry -> entry.getKey()
